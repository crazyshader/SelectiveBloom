...
half4 frag_upsample_final(v2f_multitex i) : SV_Target
{
    ...
    half4 base = tex2D(_ColorTexture, i.uvBase) * _baseIntensity;
    half3 blur = UpsampleFilter(i.uvMain);
#if UNITY_COLORSPACE_GAMMA
...
half4 frag_combiner_final(v2f_multitex i) : SV_Target
{
    half4 base = tex2D(_ColorTexture, i.uvBase) * _baseIntensity;
    half4 blur = tex2D(_MainTex, i.uvMain);
#if UNITY_COLORSPACE_GAMMA
	base.rgb = GammaToLinearSpace(base.rgb);
#endif
    //half3 cout = base.rgb + blur;
    //half3 cout = base.rgb + DecodeHDR(blur);
    half3 cout = base.rgb * (1 - blur.a) + DecodeHDR(blur) ;
#if UNITY_COLORSPACE_GAMMA
    cout.rgb = LinearToGammaSpace(cout.rgb);
#endif
	return half4(cout.rgb, blur.a);
}